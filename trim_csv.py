...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Trim a fitness-like CSV to only include columns matching a pattern (default: individual_<digits>) and always include a 'generations' column on the left if present. Writes to a 'trim_' prefixed CSV by default."
    )
    parser.add_argument(
        "input_path",
    ...
    parser.add_argument(
        "-p",
        "--pattern",
        help="Regex pattern to select columns to keep (full match). Default: ^individual_\\d+$",
        default=r"^individual_\d+$",
    )
    parser.add_argument(
        "--encoding",
